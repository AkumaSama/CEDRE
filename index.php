...
//$login = $_SERVER['HTTP_CT_REMOTE_USER'];

require("config/configBdd.php");
require("modele/M_connectBdd.php");
/*$db = getBdd($typeBdd, $host, $userBdd, $mdp, $base);
$verifLogin = $db->prepare("SELECT EXISTS(SELECT uti_login FROM utilisateur WHERE uti_login = ?);");
$verifLogin->execute(array($login));

if(!$verifLogin->fetch()[0]) {
	$insertLogin = $db->prepare("INSERT INTO utilisateur (uti_login, uti_estAdmin, uti_estSuperAdmin, stru_id) VALUES (?, false, false, 2);");
	$insertLogin->execute(array($login));
}

$getAdminPerm = $db->prepare("SELECT uti_estAdmin, uti_estSuperAdmin FROM utilisateur WHERE uti_login = ?");
$getAdminPerm->execute(array($login));
$admin = $getAdminPerm->fetchAll();

if($admin[0]['uti_estAdmin'] == "1") {
	$permAdmin = true;
} else {
	$permAdmin = false;
}

if($admin[0]['uti_estSuperAdmin'] == "1") {
	$superAdmin = true;
} else {
	$superAdmin = false;
}*/
...
<body>
	...
	<div class="container-fluid">
		...
		<div class="alert alert-success alert-dismissible position-absolute fixed-bottom w-25 ml-3">
		    ...
		    <strong>Success!</strong> Action effectué avec succès !
		</div>
		<?php
		}
		...
		if(isset($_GET['error']))
		{
			...
			<div class="alert alert-danger alert-dismissible position-absolute fixed-bottom w-50 ml-3">
    			<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
    			<strong>Erreur!</strong> <p>Une erreur c'est produite !<br> <?php echo $error; ?></p>
  			</div>
  			<?php
		}
